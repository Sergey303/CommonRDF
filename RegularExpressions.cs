...
namespace CommonRDF
{
    internal class Reg
    {
        ...
        internal static readonly Regex QuerySelect = CreateRegex(@"^\s*[Ss][Ee][Ll][Ee][Cc][Tt]\s*(((?<p>\?\S*)\s*)*|\*)\s*\s(?=\w)");
        internal static readonly Regex QueryWhere = CreateRegex(@"^\s*[Ww][Hh][Ee][Rr][Ee]\s+\{(?<insideWhere>[^{}]*(((?'Open'\{)[^{}]*)+((?'Close-Open'\})[^{}]*)+)*(?(Open)(?!)))\}");

        internal static Regex Triplet = CreateRegex(@"^\s*([\S]+)\s+([\S]+|'.*')\s+([\S]+|'.*')\s*$");
        internal static Regex TripletDot = CreateRegex(@"^\s*(([\S]+)\s+([\S]+|'.*')\s+([\S]+|'.*')\s*\.(\s+|$))+");
        internal static Regex TripletOptional = CreateRegex(
            @"^\s*[Oo][Pp][Tt][Ii][Oo][Nn][Aa][Ll]\s*{\s*(?<inside>[^{}]*(((?'Open'\{)[^{}]*)+((?'Close-Open'\})[^{}]*)+)*(?(Open)(?!)))\s*\}", RegexOptions.ExplicitCapture);
        internal static Regex Filter = CreateRegex(
            @"^\s*[Ff][Ii][Ll][Tt][Ee][Rr]\s+([^\s()]+)?\(\s*(?<filter>[^()]*(((?<Open>\()[^()]*)+((?<Close-Open>\))[^()]*)+)*(?(Open)(?!)))\s*\)");
        ...
        private static Regex CreateRegex(string pattern, RegexOptions add=RegexOptions.None)
        {
            return new Regex(pattern, add|
                RegexOptions.Singleline, TimeSpan.FromMinutes(1.0));//RegexOptions.Compiled|
        }

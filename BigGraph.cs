...
namespace CommonRDF
{
    class BigGraph : GraphBase
    {
        private PxCell indexesCell, graphPxCell;
        ...
        private void InitCells()
        {
            ...
            indexesCell = new PxCell(SearchPType, filePathIndexes);
        }

        public override void CreateGraph()
        {
            if (indexesCell != null) indexesCell.Close();
            ...
            indexesCell = new PxCell(SearchPType, filePathIndexes, false);
                indexesCell.Fill2(
                    offsetById.Select(index=>new
                                            {
        ...
        public override IEnumerable<string> GetData(string id, string predicate, object nodeInfo = null)
        {
            ...
            if (found.offset == -1 || found.IsEmpty) return Enumerable.Empty<string>();
        ...
        {
            PxEntry found = GetEntryByOffset((nodeInfo is long?) ? ((long?)nodeInfo).Value : GetOffestById(id));
            if (found.offset == -1 || found.IsEmpty) return Enumerable.Empty<PredicateDataTriple>();
            return found.Field(2).Elements()
                .Select(pRec =>
                    pRec.Field(1)
                        .Elements()
            ...
            object node = null, int? predicateSC = null)
        {
            ...
            if (found.offset == -1 || found.IsEmpty) return Enumerable.Empty<GraphTripletsTree.IdNodeInfo>();
        ...
        internal long GetOffestById(string id)
        {
            ...
            PxEntry found = indexesCell.Root.BinarySearchFirst(element =>
            {
                int v = (int)element.Field(0).Get().Value;
                return v < e_hs ? -1 : (v == e_hs ? 0 : 1);
            });
            if (found.Typ == null) return -1;
            var idOffsetEntry =
                found.Field(1)
                    .Elements()
        ...
        {
            var ar = indexesCell.Root.Elements()
                .Select(element => 
                    element.Field(1).Elements()
                                    .Select(f => f.Field(0).Get().Value)
                                    .ToArray());
        ...
        public static PType SearchPType = new PTypeSequence(
            new PTypeRecord(
                new NamedType("hash",
